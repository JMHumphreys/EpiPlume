...
[[ -n "${SUBMISSION_ID}" && -n "${N}" && -n "${ARRAY_MAP}" ]] || { echo "Could not parse map builder output" >&2; exit 2; }
if [[ "${RETRY_FAILED}" == true && "${EPIPLUME_ALLOW_FAILED_RETRY:-false}" != true ]]; then
  echo "Retries require EPIPLUME_ALLOW_FAILED_RETRY=true after inspection." >&2; exit 3
fi
EXPORTS="ALL,REPO_DIR=${REPO_DIR},CONFIG=${CONFIG},MANIFEST=${MANIFEST},ARRAY_MAP=${ARRAY_MAP},SUBMISSION_ID=${SUBMISSION_ID},EPIPLUME_ALLOW_FAILED_RETRY=${EPIPLUME_ALLOW_FAILED_RETRY:-false}"
if [[ "${DRY_RUN}" == true ]]; then
  echo "DRY RUN: sbatch --array=1-${N}%${MAX_CONCURRENT} --export=${EXPORTS} hpc/atlas_hysplit_array.sbatch"
  exit 0
fi
ARRAY_SUBMISSION="$(sbatch --array="1-${N}%${MAX_CONCURRENT}" --export="${EXPORTS}" hpc/atlas_hysplit_array.sbatch)"
ARRAY_JOB_ID="${ARRAY_SUBMISSION##* }"
COLLECT_SUBMISSION="$(sbatch --dependency="afterany:${ARRAY_JOB_ID}" --export="${EXPORTS},ARRAY_JOB_ID=${ARRAY_JOB_ID}" hpc/atlas_hysplit_array_collect.sbatch)"
